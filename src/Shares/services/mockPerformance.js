...
async function fetchUserPerformance(userId) {
    try {
        ...
        const mockUserPerformance = USER_PERFORMANCE.find(performance => performance.userId === userId);

        if(mockUserPerformance) {
          const performanceDataModels = mockUserPerformance.data.data.map(data => new PerformanceDataModel(data.value, data.kind));
          const kindElementModels = Object.entries(mockUserPerformance.data.kind).map(([id, name]) => new KindElementModel(id, name));
